...
import { useOptimizedPOSData, useOptimizedProducts } from "@/hooks/useOptimizedQueries";
...
export const OptimizedProductGrid = ({ onAddToCart }: OptimizedProductGridProps) => {
  ...
  // Usar hook optimizado para datos del POS
  const { data: posData, isLoading: posLoading } = useOptimizedPOSData();
  const categories = posData.categories || [];

  // Usar hook optimizado para productos
  const { data: products = [], isLoading: productsLoading } = useOptimizedProducts(selectedCategory);
